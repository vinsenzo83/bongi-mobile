...
function detectIntent(message) {
  ...
  // 알뜰폰 (인터넷과 혼동 방지 — 최우선 체크)
  if (/알뜰폰|알뜰|유심|mvno|저렴한.?요금/.test(q)) {
    return {
      type: 'budget_phone',
      instruction: `[시스템 강제 지시] 고객이 알뜰폰/MVNO 요금제를 물었습니다. search_products, search_rental 등 다른 도구를 절대 호출하지 마세요. 텍스트로만 알뜰폰 요금제를 안내하세요: 데이터 3GB 월 8,800원~무제한 월 19,800원, 번호이동 가능, 상담사 연결 유도.`,
    };
  }
  ...
  if (/후기|리뷰|평가|만족도/.test(q)) {
    return {
      type: 'review',
      instruction: `[시스템 강제 지시] 고객이 후기/리뷰를 요청했습니다. 반드시 search_reviews 도구만 호출하세요. category 파라미터 없이 호출하면 전체 후기를 반환합니다. 다른 도구를 호출하지 마세요.`,
    };
  }
  ...
  if (/친구.?초대|추천하면|소개하면|친구.?소개|리퍼럴|추천.?보상/.test(q)) {
    return {
      type: 'referral',
      instruction: `[시스템 강제 지시] 고객이 친구초대 보상을 물었습니다. 도구를 호출하지 말고 텍스트로 안내하세요: 1단계(친구 가입)=추천인 2,000원, 2단계(계약 완료)=추천인 20,000원 추가+친구 10,000원. 리턴캐쉬는 현금 출금 가능.`,
    };
  }
