// 단순 배치 INSERT 생성 — 짧은 answer(200자), 20행 블록
// 실행: node skt-faq-batch.mjs <startIdx> <endIdx>
...
const short = s => esc(s).slice(0, 200);
const shortA = s => esc(s).slice(0, 350);

const start = parseInt(process.argv[2]||'0', 10);
const end   = parseInt(process.argv[3]||String(raw.length), 10);
const slice = raw.slice(start, end);

// 개별 row를 "단순 INSERT" 형태로 생성
const inserts = slice.map(r => {
  const t = topicOf(r);
  const q = short(r.question);
  const a = shortA(r.answer);
  const u = short(r.source_url);
  return `INSERT INTO cs.faqs(topic_code,carrier,question,answer,source_url,confidence,crawled_at,answer_type,is_active)
SELECT '${t}','SKT','${q}','${a}','${u}','needs_review',now(),'info',true
WHERE NOT EXISTS(SELECT 1 FROM cs.faqs f WHERE f.carrier='SKT' AND(f.source_url='${u}' OR f.question='${q}'));`;
});
