// SKT FAQ INSERT v2 — valid topic_codes only (cs.topics FK constraint)
// Valid: auth, billing, bundle, cancel, device, lost, membership, micropay, mnp,
//        namechange, penalty, plan, quality, roaming, signup, sim, subsidy, vas
...
function fixTopic(r) {
  ...
  // 질문 내용 기반 고정 패턴 (catName 보다 우선)
  // T world 앱/포털 관련은 auth (T월드이용안내 카테고리 mislabel 교정)
  ...
  // 원 topic_code가 valid하면 유지
  if (VALID.has(r.topic_code)) return r.topic_code;
  return 'billing';
}

const esc = s => (s || '').replace(/'/g, "''").replace(/</g,'').replace(/>/g,'').slice(0, 750);
...
const chunkIdx = parseInt(process.argv[2] || '-1', 10);
if (chunkIdx < 0) {
  // 통계만 출력
  const dist = {};
  for (const ch of chunks) for (const r of ch) { dist[r._topic]=(dist[r._topic]||0)+1; }
  console.log(JSON.stringify({ total_chunks: chunks.length, total_rows: raw.length, dist }, null, 2));
  process.exit(0);
}

const ch = chunks[chunkIdx];
if (!ch) { console.error('No chunk', chunkIdx); process.exit(1); }

const rows = ch.map((r, ri) => {
  const cast = ri === 0 ? '::text' : '';
  const cst = ri === 0 ? '::boolean' : '';
  return `('${esc(r._topic)}'${cast},'SKT'${cast},'${esc(r.question)}'${cast},NULL${cast},'${esc(r.answer)}'${cast},NULL${cast},NULL${cast},NULL${cast},'${esc(r.source_url)}'${cast},'needs_review'${cast},now()::timestamptz,'info'${cast},true${cst})`;
}).join(',\n  ');
