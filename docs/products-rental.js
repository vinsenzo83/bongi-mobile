...
(function () {
  ...
  async function applyMargin(dry) {
    var body = { margin_pct: Number($('rp-margin').value), basis: $('rp-basis').value, supplier_id: $('rp-m-supplier').value || null, only_unset: $('rp-only-unset').checked, dry_run: dry };
    if (!dry && !confirm((body.supplier_id ? $('rp-m-supplier').selectedOptions[0].text : '전체 렌탈사') + ' 조건의 가이드·MAX 를 마진 ' + body.margin_pct + '% 규칙으로 덮어씁니다.' + (body.only_unset ? ' (미설정 조건만)' : ''))) return;
